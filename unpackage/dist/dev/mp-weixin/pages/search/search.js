...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const onSearch = () => {
      ...
      searchData();
      common_vendor.index.__f__("log", "at pages/search/search.vue:108", queryParams.value.keyword);
    };
    const onClear = () => {
      initParams();
    };
    ...
    const searchData = async () => {
      try {
        ...
        if (res.data.length == 0 && classList.value.length == 0)
          noSearch.value = true;
        common_vendor.index.__f__("log", "at pages/search/search.vue:145", res);
      } finally {
